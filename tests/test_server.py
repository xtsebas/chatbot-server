...
def test_initialize_and_tools():
    resp, st = run("initialize")
    assert "protocolVersion" in resp["result"]
    resp, st = run("tools/list", state=st)
    assert "tools" in resp["result"]

def test_reset_and_suggest_direct():
    resp, st = run("reset")
    assert resp["result"]["candidates"] > 0
    resp, st = run("suggest_move", state=st)
    assert "guess" in resp["result"]

def test_apply_feedback_and_explain_direct():
    _, st = run("reset")
...
def test_tools_call_flow():
    _, st = run("reset")
    resp, st = run("tools/call", {"name": "suggest_move", "arguments": {}}, st)
    assert "content" in resp["result"]
    resp, st = run("tools/call", {"name": "apply_feedback", "arguments": {"guess": "CRATE", "feedback": "YGYKK"}}, st)
    assert "content" in resp["result"]
